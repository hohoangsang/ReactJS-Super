...
const initialData: Omit<Post, 'id'> | Post = {
  createdAt: new Date(),
  description: '',
  featuredImage: '',
  publishDate: '',
...
export default function CreatePost() {
  const [postData, setPostData] = useState<Omit<Post, 'id'> | Post>(initialData);
  const postId = useSelector((state: RootState) => state.blog.postId);

  const dispatch = useAppDispatch();

  const [addPost, { isLoading: isAddingPost, isSuccess: isAddPostSuccess, isError: isAddPostError }] =
    useAddPostMutation();

  const [updatePost, { isLoading: isUpdatingPost, isError: isUpdatePostError, isSuccess: isUpdatePostSuccess }] =
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (postId) {
      await updatePost({ postId, post: postData as Post })
        .unwrap()
        .then(() => {
